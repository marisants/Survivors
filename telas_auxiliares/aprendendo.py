...
class Aluno(pygame.sprite.Sprite): # a segunda "Sprite" é uma classe q já é do pygame 
    def __init__(self):
        ...
        self.image = pygame.transform.scale(self.image, (128*3, 70*3))
        ...
        self.animar = False # pra animar ser false , aí se clicar em alguma tecla anima 

    def atacar(self):
        self.animar = True # aq que começa a animar , pois vira TRUE
    def update(self): # fzr update
        if self.animar == True: # só anima e for true
            ...
            if self.atual >= len(self.sprites): # pra se repetir , no caos de wuando acabar as sprits começa dnv
                self.atual = 0
                self.animar = False # pra parar de animar quando n tiver apertando , TEMOS Q TIRAR ISSO DPS PRA ELE SEMPRE ANDAR  
            self.image = self.sprites[int(self.atual)] # é pra poder botar número quebrado
            self.image = pygame.transform.scale(self.image, (128*3, 64*3)) # aumenta o tamanho da img , 128 é largura e 64 é altura 
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN: # keydown é pra quando apertar qualquer tecla do teclado
            aluno.atacar()
